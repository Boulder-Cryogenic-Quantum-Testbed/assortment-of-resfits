...
class resonator(object): # object is defined in init below
    ...
    """
    def __init__(self, freq, I, Q, pwr, delay, name = '', date = None,temp = None,bias = None):
...
#        self.corrected_power = None
    ...
    def power_calibrate(self,p): # optional input calibration: linear function of input S21(frequency) to sample cables, meas. at room temp.
        ...
        if 'DCM' in self.method : 
            ...
            Q = self.DCMparams.Q
            Qe = self.DCMparams.Qe
            self.DCMparams.num_photon = 2*p/hbar/f**2*Q**2/Qe ### calculate number of photon
        if 'INV' in self.method :
            Q = self.INVparams.Q
            Qe = self.INVparams.Qe
            self.INVparams.num_photon = 2*p/hbar/f**2*Q**2/Qe
        
    def fit(self,**kwargs):
        method_default = Fit_Method("INV")
        for key, value in kwargs.items():
            if key == 'extract_factor':
                method_default.extract_factor = value
...
"""
    def __init__(self, method,extract_factor = 1,MC_iteration = 5, MC_rounds=1e5,\
        ...
        assert (manual_init == None) or (type(manual_init)==list and len(manual_init)==6),'Wrong manual_init, None or len = 6' 
        assert (type(extract_factor) == int or type(extract_factor) == float or type(extract_factor) == list) or extract_factor == 'all' , 'Wrong extract factor, all or number'
        self.method = method
        if method == 'DCM':
        ...
        self.extract_factor = extract_factor
